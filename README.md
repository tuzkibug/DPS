...
- **批量操作**：任务列表支持多选，一键批量启动/停止/删除
- **定时运行**：支持指定任务运行时长，到时自动停止
...
### v0.3.0 (2026-05-14) — 实时监控、批量操作与运行时长控制

- **优化** Docker 后端镜像从 debian:bookworm-slim → alpine:3.20（162MB → 52MB）
- **修复** WebSocket Origin 检查，兼容 nginx 代理的 localhost 连接
- **新增** 批量任务选择，支持批量启动/停止/删除，含确认对话框
- **新增** 任务列表 QPS 迷你折线图列（SVG 坐标轴）
- **修复** 任务停止后 QPS 显示归零（后端 + 前端）
- **新增** 固定时长任务自动停止（前端新增运行时长输入）
- **新增** WebSocket 推送任务状态同步至 Zustand store，按钮即时切换
